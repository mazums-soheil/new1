(() => {
  ...
  const BUILD_VERSION = '20260914-cinema-v4-turboboot';
  const BOOT_CACHE = `mazums-assets-${BUILD_VERSION}`;
  const versioned = (path) => `${path}?v=${BUILD_VERSION}`;
  const BOOT_ASSETS = [
    { key: 'heroVideo', path: 'assets/hero-scrub-mobile.mp4', type: 'video', bytes: 4740931, priority: 100 },
  ...
  ].map((asset) => ({ ...asset, url: versioned(asset.path), received: 0, networkReceived: 0, complete: false, source: '', attempts: 0 }));
  ...
  async function fetchAssetAttempt(asset, attempt) {
    ...
    try {
      ...
      if (blob.size !== asset.bytes) throw new Error(`Asset size mismatch: ${asset.path} (${blob.size}/${asset.bytes})`);
  ...
  async function fetchAssetStrict(asset) {
    const maxAttempts = navigator.onLine === false ? 1 : 3;
    let lastError = null;
